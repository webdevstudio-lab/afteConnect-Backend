...
import { loginSchema, registerSchema, activeUserSchema } from "../validation/auth.validation";
import { HTTPSTATUS } from "../config/http.config";
import { registerUserService, loginUserService, activeUserService } from "../services/auth.service";
...
export const activeUserController = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        const {user} = await activeUserService(body , id);
...
export const logoutUserController = asyncHandler(
    async (req: Request, res: Response) => {
        ...
        return res.status(HTTPSTATUS.OK).json({message : "Vous avez bien été déconnecté"});
    }
)

// export const resetUserPasswordController = asyncHandler(
//     async (req: Request, res: Response) => {
//         const body = resetUserPasswordSchema = registerSchema.parse({...req.body});
//         const id = req.params.id

//         const user = await resetUserPasswordService(body , id);
