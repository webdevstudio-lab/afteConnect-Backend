...
export const registerUserService = async(body: {
    ...
    const {fullname, email, password, poste} = body
        ...
        if(existingUser) {
            throw new BadRequestException('cette addresse email est utilisé par un autre compte',
...
export const activeUserService = async(body: { newPassword: string},id: string) => {
    ...
    const isPasswordCorrect = await compareValue(newPassword, existingUser.password)
    if(isPasswordCorrect) {
        throw new BadRequestException("Vous ne pouvez pas utiliser le mot de passe par default",
                 ErrorCodeEnum.AUTH_INVALID_CREDENTIALS)
    }
